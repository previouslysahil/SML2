...
extension FTensor {
    ...
    public func pool2D_max(size: Int) -> FTensor {
        var res = FTensor(shape: [shape[0] - size + 1, shape[1] - size + 1], repeating: 0)
        var idx = 0
        for r in 0..<shape[0] {
...
extension FTensor {
    ...
    public func sum(axis: Int, keepDim: Bool = false) -> FTensor {
        ...
        if shape.main.count == 1 {
            return FTensor(shape: newShape, grid: [self.sum()])
        } else if shape.main.count == 2 {
            var t = FTensor(shape: newShape, repeating: 0)
            grid.withUnsafeBufferPointer { gridPtr in
                t.grid.withUnsafeMutableBufferPointer { tPtr in
                    if axis == 0 {
                        for a in 0..<shape.main[1] {
                            vDSP_sve(gridPtr.baseAddress! + a, shape.main[1], tPtr.baseAddress! + a, vDSP_Length(shape.main[0]))
                        }
                    } else {
                        for a in 0..<shape.main[0] {
                            vDSP_sve(gridPtr.baseAddress! + a * shape.main[1], 1, tPtr.baseAddress! + a, vDSP_Length(shape.main[1]))
                        }
                    }
                }
            }
    ...
    public func zscore_image() -> (norm: FTensor, mean: FTensor, std: FTensor) {
        ...
        var t = self
        var mean = FTensor(shape: [shape[1], shape[2], shape[3]], repeating: 0)
        var std = FTensor(shape: [shape[1], shape[2], shape[3]], repeating: 0)
        mean.grid.withUnsafeMutableBufferPointer { meanPtr in
            std.grid.withUnsafeMutableBufferPointer { stdPtr in
                ...
                for p in 0..<image_size {
                    t.grid.withUnsafeMutableBufferPointer { tPtr in
                        grid.withUnsafeBufferPointer { gridPtr in
                            vDSP_normalize(gridPtr.baseAddress! + p, image_size, tPtr.baseAddress! + p, image_size, meanPtr.baseAddress! + p, stdPtr.baseAddress! + p, vDSP_Length(shape[0]))
                        }
                    }
                }
            }
